...
import { Log } from '../controls/judgelog'
import 'mocha'
...
describe('Log', () => {
  describe('#parse', () => {
    it('Should parse a normal log without error', done => {
      readFile(
        join(process.cwd(), 'testdata', 'sample-log.log'),
        'utf-8',
        (err, file) => {
          if (err) return done(err)
          const f = Log.parse('asdf0001', 'LINES', file)
          assert.deepEqual(
            f,
            {
              verdict: 100,
              details: [
                ...
                },
                {
                  ...
                  time: 0,
                  verdict:
                    'Chạy sinh lỗi: Command: "C:\\ProgramData\\ThemisWorkSpace\\ContestRoom\\VO17BACH.exe" terminated with exit code: 3221225477 (Hexadecimal: C0000005)'
                },
                {
                  id: 'Test005',
                  score: 0,
    ...
    it('Should parse a compile-error log without error', done => {
      readFile(
        join(process.cwd(), 'testdata', 'compile-error.log'),
        'utf-8',
        (err, file) => {
          if (err) return done(err)
          const f = Log.parse('dangcuong_123', 'HIGHWAY', file)
          assert.deepEqual(
            f,
            {
              verdict: 'ℱ Dịch lỗi',
    ...
    it('Should parse a normal log with VN regional settings without error', done => {
      readFile(
        join(process.cwd(), 'testdata', 'vn-log.log'),
        'utf-8',
        (err, file) => {
          if (err) return done(err)
          const f = Log.parse('asdf0001', 'LINES', file)
